...
public class ClassInitializer {
	...
	public static void initializeRuntime() {
		...
		final File extDir = new File(javaHome + fileSeparator + "lib" + fileSeparator + "ext");
	...
	private static void initialize(String prefix, File file) {
		...
		if(file.isDirectory()) {
			final File[] files = file.listFiles();
			final String newPrefix = prefix.length() == 0 ? name : prefix + "." + name;
			final int len = files.length;
			for(int i = 0; i < len; ++i) {
			...
			final String className = prefix + "." + name.substring(0, name.length() - 6);
			ClassInitializer.initialize(className);
		}
	}
}