...
public abstract class Enum implements Comparable, Serializable {
	...
	protected Enum(String name, int ordinal) {
		...
		synchronized(CLASS_TO_ENUMS) {
			MemoryArea.getMemoryArea(CLASS_TO_ENUMS).executeInArea(new Runnable() {
				public void run() {
					FastMap nameToEnum = (FastMap) CLASS_TO_ENUMS.get(Enum.this.getClass());
					if(nameToEnum == null) {
						nameToEnum = new FastMap();
						CLASS_TO_ENUMS.put(Enum.this.getClass(), nameToEnum);
					}
					final Object prev = nameToEnum.put(_name, Enum.this);
					if(prev != null)
						throw new IllegalArgumentException("Duplicate enum " + _name);
				}
			});
		}
	}
