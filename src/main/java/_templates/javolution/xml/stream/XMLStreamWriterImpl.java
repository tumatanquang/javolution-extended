		...
		for(int i = i0; i < i2; i++) {
			if(_isRepairingNamespaces && i < i1 && !_namespaces._prefixesWritten[i]
				...
				if(_isRepairingNamespaces) {
					final CharArray prefix = _namespaces.getPrefix(_namespaces._namespaces[i], i);
	...
	private final void write(Object csq, int start, int length, boolean escapeMarkup) throws XMLStreamException {
		if(_index + length <= BUFFER_LENGTH) { // Enough buffer space.
			if(csq instanceof String) {
				...
				((_templates.javolution.text.Text) csq).getChars(start, start + length, _buffer, _index);
			}
			else if(csq instanceof _templates.javolution.text.TextBuilder) {
				((_templates.javolution.text.TextBuilder) csq).getChars(start, start + length, _buffer, _index);
			}
			else if(csq instanceof _templates.javolution.text.CharArray) {
				((_templates.javolution.text.CharArray) csq).getChars(start, start + length, _buffer, _index);
			}
			else {
