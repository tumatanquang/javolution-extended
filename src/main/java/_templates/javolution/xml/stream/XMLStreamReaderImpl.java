...
public final class XMLStreamReaderImpl implements XMLStreamReader, Reusable {
	 ...
	 */
	private static final String[] NAMES_OF_EVENTS = new String[] {"UNDEFINED", "START_ELEMENT", "END_ELEMENT",
	...
	public void setInput(Reader reader) throws XMLStreamException {
		...
		try { // Reads prolog (if there)
			final int readCount = reader.read(_readBuffer, _startOffset, _readBuffer.length - _startOffset);
	...
	private char replaceEntity() throws XMLStreamException {
		...
		// Replaces the entity.
		final int length = _entities.replaceEntity(_data, start, _index - start);
	...
	private void processAttribute() throws XMLStreamException {
		if(_attrPrefixSep < 0) { // No prefix.
			...
			final CharArray localName = newSeq(_attrPrefixSep + 1, offset + length - _attrPrefixSep - 1);
	...
	private CharArray newSeq(int offset, int length) {
		final CharArray seq = _seqsIndex < _seqsCapacity ? _seqs[_seqsIndex++] : newSeq2();
	...
	private void increaseDataBuffer() {
		...
		MemoryArea.getMemoryArea(this).executeInArea(new Runnable() {
			public void run() {
				final char[] tmp = new char[_data.length * 2];
				_templates.javolution.context.LogContext
	...
	private void increaseStack() {
		MemoryArea.getMemoryArea(this).executeInArea(new Runnable() {
			public void run() {
				final CharArray[] tmp = new CharArray[_elemStack.length * 2];
				_templates.javolution.context.LogContext
	...
	public CharArray getLocalName() {
		...
		if(_prefixSep < 0)
			return _qName;
		final CharArray localName = newSeq(_prefixSep + 1, _qName.offset() + _qName.length() - _prefixSep - 1);
		return localName;
	}
	...
	public CharArray getPITarget() {
		if(_eventType != XMLStreamConstants.PROCESSING_INSTRUCTION)
			throw illegalState("Not a processing instruction");
		final CharArray piTarget = newSeq(_text.offset(), _text.indexOf(' ') + _text.offset());
		return piTarget;
	}
