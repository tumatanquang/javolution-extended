 ...
 */
public final class SharedFastTable/*<E>*/ extends FastTable/*<E>*/ implements List/*<E>*/, Reusable, RandomAccess {
	private final ReadWriteLock _lock;
	public SharedFastTable(FastTable/*<E>*/ inner) {
		super(inner);
			...
			try {
				...
				for(;;) {
					try {
						r.acquire();
						try {
							return _nextIndex < SharedFastTable.super.size();
						}
						finally {
							r.release();
						}
			...
			try {
				...
				for(;;) {
					try {
						r.acquire();
						try {
							if(_nextIndex >= SharedFastTable.super.size())
								throw new NoSuchElementException();
							_currentIndex = _nextIndex;
							return SharedFastTable.super.get(_nextIndex++);
						}
						finally {
							r.release();
						}
			...
			try {
				...
				for(;;) {
					try {
						r.acquire();
						try {
							return _nextIndex > 0 && _nextIndex <= SharedFastTable.super.size();
						}
						finally {
							r.release();
						}
				...
				for(;;) {
					try {
						...
						try {
							...
							if(idx >= SharedFastTable.super.size())
								throw new NoSuchElementException();
							_currentIndex = idx;
							return SharedFastTable.super.get(idx);
						}
						finally {
							r.release();
						}
				...
				for(;;) {
					try {
						...
						try {
							...
							SharedFastTable.super.remove(_currentIndex);
				...
				for(;;) {
					try {
						...
						try {
							if(_currentIndex < 0)
								throw new IllegalStateException();
							SharedFastTable.super.set(_currentIndex, value);
						}
						finally {
							w.release();
						}
			...
			try {
				...
				for(;;) {
					try {
						w.acquire();
						try {
							SharedFastTable.super.add(_nextIndex++, value);
							_currentIndex = -1;
						}
						finally {
							w.release();
