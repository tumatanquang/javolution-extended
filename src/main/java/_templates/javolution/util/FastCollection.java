...
public abstract class FastCollection/*<E>*/ implements Collection/*<E>*/, Iterable/*<E>*/, XMLSerializable, Realtime {
	...
	public boolean addAll(Collection/*<? extends E>*/ values) {
		...
		for(final Iterator /*<? extends E>*/ itr = values.iterator(); itr.hasNext();) {
			if(add(itr.next())) {
				modified = true;
			}
		}
	...
	public boolean removeAll(Collection/*<?>*/ values) {
		...
		for(Record head = head(), r = tail().getPrevious(), previous; r != head; r = previous) {
			...
			if(FastCollection.contains(values, valueOf(r), getValueComparator())) {
				delete(r);
				modified = true;
			}
		}
	...
	private static final class Shared extends FastCollection implements Collection, Serializable {
		private final FastCollection fc; // Backing FastCollection
		private final Object mutex; // Object on which to synchronize
		private Shared(FastCollection target) {
			if(target == null)
		...
		private final class ListArrayIterator implements Iterator {
			...
			public void remove() {
				...
				final Object removed = _elements[_index - 1];
				if(removed == NULL)
					throw new IllegalStateException();
		...
		private final class CollectionArrayIterator implements Iterator {
			private final Object[] _elements;
			private int _index;
