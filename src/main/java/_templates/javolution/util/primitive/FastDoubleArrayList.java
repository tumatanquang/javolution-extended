 ...
 * @since 5.7.1
 */
public strictfp class FastDoubleArrayList implements Cloneable, RandomAccess, Serializable {
	...
	public Object/*FastDoubleArrayList*/ clone() throws CloneNotSupportedException {
		/*@JVM-1.1+@
		if(true) {
	...
	public boolean addAll(int index, double[] values) {
		...
		if(moved > 0) {
			System.arraycopy(elementData, index, elementData, index + values.length, moved);
		}
	...
	public boolean addAll(int index, FastDoubleArrayList values) {
		...
		final int moved = size - index;
		if(moved > 0) {
			System.arraycopy(elementData, index, elementData, index + values.size, moved);
		}
		System.arraycopy(values.elementData, 0, elementData, index, values.size);
		size += values.size;
		return true;
	}
	...
	public boolean equals(Object o) {
		...
		if(size != that.size)
			return false;
		for(int i = -1; ++i < size;) {
			if(Double.doubleToLongBits(elementData[i]) != Double.doubleToLongBits(that.elementData[i]))
				return false;
		}
		return true;
	}
