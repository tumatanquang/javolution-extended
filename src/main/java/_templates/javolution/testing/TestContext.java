...
public abstract class TestContext extends LogContext {
	...
	public static void run(TestSuite testSuite) throws Exception {
		final TestContext testContext = (TestContext) LogContext.getCurrentLogContext();
	...
	public static boolean assertEquals(Object expected, Object actual, CharSequence message) {
		final boolean ok = expected == null && actual == null || expected != null && expected.equals(actual);
	...
	public static boolean assertEquals(double expected, double actual, CharSequence message) {
		final boolean ok = expected == actual || Double.isNaN(expected) && Double.isNaN(actual);
		message = ok || message != null ? message : // Provides error message if necessary.
				Text.valueOf(expected + " expected but found " + actual);
		final TestContext ctx = (TestContext) LogContext.getCurrentLogContext();
		return ctx.doAssert(ok, message);
