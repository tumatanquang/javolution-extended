...
public abstract class StackContext extends AllocatorContext {
	...
	private static final class Default extends StackContext {
		...
		protected Allocator getAllocator(ObjectFactory factory) {
			final FastMap factoryToAllocator = (FastMap) _factoryToAllocator.get();
			...
			if(allocator.user == null) { // Activate.
				...
				final FastTable activeAllocators = (FastTable) _activeAllocators.get();
		...
		protected void exitAction() {
			...
			for(int i = 0; i < size; ++i) {
				final StackAllocator allocator = (StackAllocator) _ownerUsedAllocators.get(i);
				allocator.reset();
			}
			_ownerUsedAllocators.clear();
			size = _nonOwnerUsedAllocators.size();
			for(int i = 0; i < size; ++i) {
				final StackAllocator allocator = (StackAllocator) _nonOwnerUsedAllocators.get(i);
				allocator.reset();
			}
