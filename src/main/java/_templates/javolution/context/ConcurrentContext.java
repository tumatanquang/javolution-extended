...
public abstract class ConcurrentContext extends Context {
	...
	private static int availableProcessors() {
		...
		if(availableProcessors != null) {
			final Integer processors = (Integer) availableProcessors.invoke(Runtime.getRuntime());
	...
	public static void execute(Runnable logic) {
		final ConcurrentContext ctx = (ConcurrentContext) Context.getCurrentContext();
	...
	static final class Default extends ConcurrentContext {
		 ...
		 */
		private static final ConcurrentThread[] _Executors = new ConcurrentThread[((Integer) MAXIMUM_CONCURRENCY.get())
				.intValue()];
		static {
			final int length = Default._Executors.length;
