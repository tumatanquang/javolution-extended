...
public final class ByteBuffer extends Buffer {
	...
	public long getLong(int index) {
		...
		return ((long) _bytes[index] << 56) + ((_bytes[++index] & 0xffL) << 48) + ((_bytes[++index] & 0xffL) << 40)
				+ ((_bytes[++index] & 0xffL) << 32) + ((_bytes[++index] & 0xffL) << 24)
				+ ((_bytes[++index] & 0xff) << 16) + ((_bytes[++index] & 0xff) << 8) + (_bytes[++index] & 0xffL);
	}
