...
public class JavolutionTest extends junit.framework.TestSuite {
	public static junit.framework.Test suite() {
		...
		for(final TestCase test : new TypeFormatTestSuite().tests()) {
			suite.addTest(new JUnitTestCase(test));
		}
		for(final TestCase test : new ContextTestSuite().tests()) {
			suite.addTest(new JUnitTestCase(test));
		}
		for(final TestCase test : new StructTestSuite().tests()) {
			suite.addTest(new JUnitTestCase(test));
		}
		// ...
		return suite;
